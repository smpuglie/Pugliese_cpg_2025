...
def run(W, tAxis, T, dt, inputs, tau, threshold, a, frCap, excSynapseMultiplier, inhSynapseMultiplier, r_tol=1e-7, a_tol=1e-9):
    """run the simulation"""
    ...
    odeSolution = diffeqsolve(term, solver, 0, T, dt, R0, 
                              args=(tAxis,inputs,tau,Wt_reweighted,threshold,a,frCap), saveat=saveat,
...
def simulate(W, args):
    ...
    Rs = vmap(run, in_axes=vmap_axes)(W, tAxis, T, dt, input, tau, threshold, a, frCap, excSynapseMultiplier, inhSynapseMultiplier)
    end_time = time.time()
    print('Time to solve system vmap: ' + str(end_time-start_time))

    del W, tAxis, T, dt, input, tau, threshold, a, frCap, excSynapseMultiplier, inhSynapseMultiplier, args
    gc.collect()
...
def generate_params(wTable, nSims, nNeurons, stimNeurons, seeds):
    ...
    input = pulse_input(0.02,0.9,stimI,stimNeurons, nNeurons, nSims, tAxis, dt)
    if seeds is not None:
    ...
    return (tAxis, T, dt, input, tau, threshold, a, frCap, excSynapseMultiplier, inhSynapseMultiplier)
...
def generate_params_from_config(wTable, params, nSims, nNeurons, stimNeurons):
    ...
    input = pulse_input(params["pulseStart"],params["pulseEnd"],params["stimI"],
                        stimNeurons, nNeurons, nSims, tAxis, dt, iterStimNeurons=iterStimNeurons)
