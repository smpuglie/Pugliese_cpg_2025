...
def run(W, tAxis, T, dt, inputs, pulseStart, pulseEnd, tau, threshold, a, frCap, seed, 
        excDnIdxs, inhDnIdxs, excInIdxs, inhInIdxs, mnIdxs, r_tol=1e-7, a_tol=1e-9):
    """run the simulation"""
    ...
    W_rw = full_shuffle(W, seed, excDnIdxs, inhDnIdxs, excInIdxs, inhInIdxs, mnIdxs)
...
def simulate(W, args):
    tAxis, T, dt, input, pulseStart, pulseEnd, tau, threshold, a, frCap, excSynapseMultiplier, inhSynapseMultiplier, seed, excDnIdxs, inhDnIdxs, excInIdxs, inhInIdxs, mnIdxs = args
    ...
    print(tau.shape, threshold.shape, a.shape, frCap.shape, seed.shape)
    
    vmap_axes = (None, None, None, None, None, None, None, 0, 0, 0, 0, 0, None, None, None, None, None)
    start_time = time.time()
    print('run')
    Rs = vmap(run, in_axes=vmap_axes)(W_rw, tAxis, T, dt, input, pulseStart, pulseEnd, tau, threshold, a, frCap, seed, excDnIdxs, inhDnIdxs, excInIdxs, inhInIdxs, mnIdxs)
    end_time = time.time()
    print('Time to solve system vmap: ' + str(end_time-start_time))

    del W_rw, tAxis, T, dt, input, tau, threshold, a, frCap, excSynapseMultiplier, inhSynapseMultiplier, seed, excDnIdxs, inhDnIdxs, excInIdxs, inhInIdxs, mnIdxs, args
    gc.collect()
...
def shuffle_W(W, seed, idxs, independent: bool = False):
    key = random.key(seed)
    return W.at[:,idxs].set(random.permutation(key, W[:,idxs], axis=1, independent=False))

def full_shuffle(W, seedOfSeeds, excDnIdxs, inhDnIdxs, excInIdxs, inhInIdxs, mnIdxs, independent:bool = False):
...
def generate_params_from_config(wTable, params, nSims, nNeurons, stimNeurons):
    ...
    excInIdxs = jnp.array(wTable.loc[~(wTable["class"].isin(["descending neuron","motor neuron"]))
                        & (wTable["predictedNt"]=="acetylcholine")].index)
    inhInIdxs = jnp.array(wTable.loc[~(wTable["class"].isin(["descending neuron","motor neuron"]))
                        & ~(wTable["predictedNt"]=="acetylcholine")].index)
    mnIdxs = jnp.array(wTable.loc[wTable["class"]=="motor neuron"].index)
    ...
    return (tAxis, T, dt, input, pulseStart, pulseEnd, tau, threshold, a, frCap, excSynapseMultiplier, inhSynapseMultiplier, params["seed"], excDnIdxs, inhDnIdxs, excInIdxs, inhInIdxs, mnIdxs)
