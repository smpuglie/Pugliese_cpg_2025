...
#import src.vncNetSparse as vncNetSparse
import pandas as pd
import sparse
...
def setup_sim_from_params(params):
    ...
    #TODO: handle if we are iterating remove
    
    if "removeNeurons" in params["paramsToIterate"]:
        Ws = np.tile(W, (nSims, 1, 1))
        for sim in range(nSims):
            removeNeurons = np.array(params["removeNeurons"][sim])
            if len(removeNeurons)>0:
                Ws[sim] = vncNetParallel.silence_neurons(Ws[sim], removeNeurons)
    elif len(params["removeNeurons"]) == nSims:
        print('hi')
    elif len(params["removeNeurons"]) > 0:
        removeNeurons = np.array(params["removeNeurons"])
        W = vncNetParallel.silence_neurons(W, removeNeurons)
    ...
    params_arr = vncNetParallel.generate_params_from_config(wTable, params, nSims, nNeurons, stimNeurons)
    if "removeNeurons" in params["paramsToIterate"]:
        return Ws, params_arr, nSims, stimNeurons
    else:
...
def run_sim_from_params(params):
    ...
    if "adjustStimI" in params: # in here for now for older configs without "adjustStimI"
        if params["adjustStimI"]:
            ...
            Rs, stimIs = vncNetParallel.run_with_stim_adjustment(Ws, params_arr, nSims, stimNeurons,
            ...
            Rs = vncNetParallel.simulate(Ws, params_arr)
    else:
        Rs = vncNetParallel.simulate(Ws, params_arr)
