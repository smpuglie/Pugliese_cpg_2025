...
from src.dt_vnc_rnn.vnc_network import process_parameter_stats as process_parameter_stats_numpy
...
class VNCRecurrentNetwork:
    ...
    def simulate(
        ...
        """
        # Pre-compute all data to avoid tracing issues in scan
        input_data = input_func.get_full_input_data()
        noise_input_data = noise_func_input.get_full_noise_data()
        noise_recurrent_data = noise_func_recurrent.get_full_noise_data()

        # Use the JIT-compiled method with pre-computed data
        return self.simulate_jit(
            num_timesteps, input_data, noise_input_data, noise_recurrent_data, initial_state, batch_size
        )
    ...
    def simulate_jit(
        ...
        num_timesteps: int,
        input_data: Array,
        noise_input_data: Array,
        noise_recurrent_data: Array,
        initial_state: Optional[Array] = None,
        batch_size: int = 1,
    ) -> Array:
        """
        ...
        Args:
            ...
            noise_input_data: Pre-computed input noise data,
                            shape (num_timesteps, batch_size, num_neurons) or broadcastable
            noise_recurrent_data: Pre-computed recurrent noise data,
                                shape (num_timesteps, batch_size, num_neurons) or broadcastable
            initial_state: Initial firing rates. For single sim: shape (N,), for batch: shape (batch_size, N)
            batch_size: Number of parallel simulations (default 1 for single simulation)
        ...
        def simulation_step(carry, inputs):
            ...
            r_current = carry
            input_t, noise_input_t, noise_recurrent_t = inputs
            ...
            r_new = self.one_minus_alpha * r_current + self.alpha * activated

            return r_new, r_new

        # Stack the input arrays for scan
        scan_inputs = (input_data, noise_input_data, noise_recurrent_data)

        # Run the simulation using JAX scan
        final_state, all_states = lax.scan(simulation_step, r, scan_inputs)
...
def create_random_network(
    ...
    if numpy_random:
        ...
        # Random parameters
        gains = jnp.asarray(process_parameter_stats_numpy(num_neurons, gains_stats))
        max_rates = jnp.asarray(process_parameter_stats_numpy(num_neurons, max_firing_rates_stats))
        thresholds = jnp.asarray(process_parameter_stats_numpy(num_neurons, thresholds_stats))
        time_constants = jnp.asarray(process_parameter_stats_numpy(num_neurons, time_constants_stats))
    else:
        # Create base key and split for different parameter types
        base_key = random.PRNGKey(random_seed if random_seed is not None else 0)
