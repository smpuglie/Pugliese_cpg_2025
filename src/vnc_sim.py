...
def update_single_sim_state(state, R, mn_mask, oscillation_threshold=0.5, clip_start=250):
    ...
    removed_stim_continue = jax.lax.select(
        need_new_round_continue, removed_stim_neurons, removed_stim_neurons.at[neuron_idx_continue].set(True)
    )
    total_removed_continue = jax.lax.select(
        need_new_round_continue, total_removed_continue, total_removed_continue.at[neuron_idx_continue].set(True)
    )
    ...
    last_removed_continue = jax.lax.select(
        need_new_round_continue, last_removed_continue, last_removed_continue.at[neuron_idx_continue].set(True)
    )
    ...
    prev_put_back_continue = jax.lax.select(
        need_new_round_continue, neurons_put_back, prev_put_back  # Current becomes previous
    )
    ...
    removed_stim_reset = jax.lax.select(
        need_new_round_reset, removed_stim_reset, removed_stim_reset.at[neuron_idx_reset].set(True)
    )
    total_removed_reset = jax.lax.select(
        need_new_round_reset, total_removed_reset, total_removed_reset.at[neuron_idx_reset].set(True)
    )
    ...
    prev_put_back_reset = jax.lax.select(
        need_new_round_reset, neurons_put_back, prev_put_back  # Current becomes previous
    )
...
def reshape_state_for_pmap(state: Pruning_state, n_devices: int) -> Pruning_state:
    ...
    return Pruning_state(
        ...
        total_removed_neurons=state.total_removed_neurons.reshape(
            n_devices, batch_per_device, *state.total_removed_neurons.shape[1:]
        ),
        removed_stim_neurons=state.removed_stim_neurons.reshape(
            n_devices, batch_per_device, *state.removed_stim_neurons.shape[1:]
        ),
...
def pad_state_for_devices(state: Pruning_state, target_batch_size: int) -> Pruning_state:
    ...
    total_removed_neurons_pad = jnp.zeros(
        (pad_size, *state.total_removed_neurons.shape[1:]), dtype=state.total_removed_neurons.dtype
    )
    removed_stim_neurons_pad = jnp.zeros(
        (pad_size, *state.removed_stim_neurons.shape[1:]), dtype=state.removed_stim_neurons.dtype
    )
...
@jit
def process_batch_Wmask(neuron_params: NeuronParams, sim_params: SimParams, batch_indices: jnp.ndarray) -> jnp.ndarray:
...
def process_batch_shuffle(
    neuron_params: NeuronParams, sim_params: SimParams, batch_indices: jnp.ndarray
) -> jnp.ndarray:
    ...
    def single_sim(idx):
        ...
        stim_idx = idx // sim_params.n_param_sets
        shuffle_indices = (
            neuron_params.exc_dn_idxs,
            neuron_params.inh_dn_idxs,
            neuron_params.exc_in_idxs,
            neuron_params.inh_in_idxs,
...
@jit
def process_batch_noise(neuron_params: NeuronParams, sim_params: SimParams, batch_indices: jnp.ndarray) -> jnp.ndarray:
...
def calculate_optimal_batch_size(
    n_neurons: int, n_timepoints: int, n_replicates: int, n_devices: int = None, max_batch_size: int = 256
) -> int:
    """
...
def initialize_pruning_state(neuron_params: NeuronParams, sim_params: SimParams, batch_size: int) -> Pruning_state:
...
def run_simulation_engine(
    neuron_params: NeuronParams, sim_params: SimParams, sim_config: SimulationConfig
    ...
    print(
        f"Running {total_sims} {sim_config.sim_type} simulations with batch size {batch_size} on {n_devices} device(s)"
    )

    # Handle pruning setup
    if sim_config.enable_pruning:
        return _run_with_pruning(neuron_params, sim_params, sim_config, batch_func, total_sims, batch_size, n_devices)
    else:
...
def _run_stim_neurons(
    ...
    for i in range(n_batches):
        ...
        if n_devices > 1 and len(batch_indices) < batch_size:
            ...
            batch_indices = jnp.concatenate([batch_indices, jnp.repeat(batch_indices[-1], pad_size)])
    ...
    return (
        results.reshape(
            sim_params.n_stim_configs, sim_params.n_param_sets, sim_params.n_neurons, len(sim_params.t_axis)
        ),
        None,
        adjusted_neuron_params,
    )
...
def _run_with_pruning(
    ...
    batch_update = jax.vmap(jitted_update, in_axes=(0, 0, None))  # state, R, mn_mask - all batched
    ...
    for i in range(n_batches):
        ...
        # Move results to CPU to save GPU memory
        batch_results = jax.device_put(batch_results, jax.devices("cpu")[0])
        mini_circuit = jnp.stack(
            [((jnp.sum(batch_results[:, n], axis=-1) > 0) & ~mn_mask) for n in range(batch_results.shape[1])], axis=1
        )
...
def prepare_neuron_params(cfg: DictConfig, W_table: Any, param_path: Optional[Path] = None) -> NeuronParams:
    ...
    tau = sample_trunc_normal(keys[0], cfg.neuron_params.tauMean, cfg.neuron_params.tauStdv, (num_sims, n_neurons))
    a = sample_trunc_normal(keys[1], cfg.neuron_params.aMean, cfg.neuron_params.aStdv, (num_sims, n_neurons))
    threshold = sample_trunc_normal(
        keys[2], cfg.neuron_params.thresholdMean, cfg.neuron_params.thresholdStdv, (num_sims, n_neurons)
    )
    fr_cap = sample_trunc_normal(
        keys[3], cfg.neuron_params.frcapMean, cfg.neuron_params.frcapStdv, (num_sims, n_neurons)
    )
...
def run_vnc_simulation(cfg: DictConfig) -> Union[jnp.ndarray, Tuple[jnp.ndarray, Pruning_state]]:
    ...
    if getattr(cfg.experiment, "dn_screen", False):
        all_exc_dns = W_table.loc[
            (W_table["class"] == "descending neuron") & (W_table["predictedNt"] == "acetylcholine")
        ]
